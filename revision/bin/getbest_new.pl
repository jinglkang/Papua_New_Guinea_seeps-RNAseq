...
`mkdir $resultnf` unless -d $resultnf;


my ($QUERY_FILE, $FASTA_FILENF, $FASTA_FILEF);
open $QUERY_FILE, "< $query.fa" or die ("Cannot open $query for reading ($!)");
while (my $line = readline ($QUERY_FILE)) {
    chomp $line;
    if ((my $gene) = $line =~ /^>(\S+)/) { # if we find >
        my $newnf = "result/$resultdirnf/$gene.$subject.fas"; #create files for each query genes
