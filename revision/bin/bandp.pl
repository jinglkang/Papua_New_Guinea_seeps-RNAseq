...
my $queryfile = "$query.fa"; #get the name of blast query file
foreach my $sub (@sortedsubgenome) {
    my $databasefile = "$sub";
    my $blastout = "$query.$sub.blast.txt";
    print "Blasting, $query against $sub...\n";
    `blastn -query "$queryfile" -task blastn -db "$databasefile" -out "$blastout" -word_size 7 -gapopen 5 -gapextend 2 -penalty -1 -reward 1 -evalue $E -outfmt 6 -num_threads 20`;
